...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - x) > bh) {
		...
		/* 	/1* fix overflow when window name is bigger than window width *1/ */
		/* 	int mid = (m->ww - (int)TEXTW(m->sel->name)) / 2 - x; */
		/* 	/1* make sure name will not overlap on tags even when it is very long *1/ */
		/* 	mid = mid >= lrpad / 2 ? mid : lrpad / 2; */
		/* 	drw_setscheme(drw, scheme[SchemeNorm]); */
		/* 	/1* drw_setscheme(drw, scheme[m == selmon ? SchemeNorm : SchemeNorm]); *1/ */
		/* 	drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0); */
		/* 	/1* drw_text(drw, x, 0, w, bh, mid, "", 0); *1/ */
		/* 	if (m->sel->isfloating) */
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	// This patch makes it so if only 1 window on a tag, no border is drawn
